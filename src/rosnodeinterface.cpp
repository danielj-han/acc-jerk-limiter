  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"ready_controller\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(ready_controller_M, (NULL));
        ready_controller_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(ready_controller_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(ready_controller_M)
                == (NULL)) || rtmGetStopRequested(ready_controller_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(ready_controller_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
